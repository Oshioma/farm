...
const blankForm = { date: "", location: "", action: "", result: "", notes: "" };
...
export default function SoilTestsPage() {
  ...
  const [modal, setModal] = useState<SoilTestEntry | null | "new">(null);
  ...
  async function loadEntries(farmId: string) {
    const rows = await getSoilTests(farmId);
    setEntries(rows);
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (modal === "new") {
        const { error: e } = await supabase.from("soil_tests").insert(payload);
        if (e) throw e;
      } else if (modal) {
        const { error: e } = await supabase.from("soil_tests").update(payload).eq("id", (modal as SoilTestEntry).id);
        if (e) throw e;
      }
  ...
  async function handleDelete(id: string) {
    try {
      setDeletingId(id);
      const { error: e } = await supabase.from("soil_tests").delete().eq("id", id);
      if (e) throw e;
  ...
  function openEdit(entry: SoilTestEntry) {
    setForm({
      date: entry.date ?? "",
                ...
                <tbody>
                  {entries.map((row) => (
                    <tr key={row.id} className="border-b border-zinc-100 last:border-b-0 hover:bg-zinc-50 align-top transition-colors">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap font-medium">{row.location ?? <span className="text-zinc-300">&mdash;</span>}</td>
                      <td className="px-4 py-3 text-zinc-600">{row.action ?? <span className="text-zinc-300">&mdash;</span>}</td>
