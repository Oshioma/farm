      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <header className="mb-6 rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <h1 className="mt-1 text-3xl font-semibold tracking-tight">{t("Soil improvements")}</h1>
        ...
        {loading ? (
          ...
          <div className="rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm text-center text-sm text-zinc-500">
            {t("No soil improvement entries yet.")}
          </div>
        ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-lg rounded-3xl border border-zinc-200 bg-white p-6 shadow-xl">
            <h2 className="mb-5 text-lg font-semibold">
              {modal === "new" ? t("Add soil improvement") : t("Edit — {name}", { name: (modal as SoilImprovement).bed ?? t("entry") })}
            </h2>
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-zinc-600">{t("Bed")}</label>
                  <input className="w-full rounded-xl border border-zinc-300 px-3 py-2 text-sm outline-none focus:border-zinc-900" value={form.bed} onChange={(e) => setForm((p) => ({ ...p, bed: e.target.value }))} placeholder={t("Tomato bed, Herb spiral…")} />
                </div>
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-medium text-zinc-600">{t("Method")}</label>
                <input className="w-full rounded-xl border border-zinc-300 px-3 py-2 text-sm outline-none focus:border-zinc-900" value={form.method} onChange={(e) => setForm((p) => ({ ...p, method: e.target.value }))} placeholder={t("Mulching, compost tea, cover crop…")} />
              </div>
              <div>
