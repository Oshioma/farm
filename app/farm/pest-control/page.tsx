...
export default function PestControlPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(errMsg(err, t("Failed to save entry")));
    } finally {
      setSaving(false);
    }
  }
  ...
  function zoneNamesFor(entry: PestControlEntry): string {
    ...
    return ids
      .map((id) => zones.find((z) => z.id === id)?.name ?? t("Unknown zone"))
      .join(", ");
  }
        ...
        <header className="mb-6 rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <h1 className="mt-1 text-3xl font-semibold tracking-tight">{t("Pest control log")}</h1>
          ...
          <p className="mt-3 text-sm text-zinc-500">
            {t("Every spray or treatment you apply to a bed. The beds map shows these dates, so you can see at a glance how many times a bed has been treated.")}
          </p>
        </header>
        ...
        <div className="mb-6">
          ...
          {showForm && (
            <div className="mt-4 max-w-lg rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
              <h2 className="text-lg font-semibold">{t("New pest control entry")}</h2>
              <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="mb-2 block text-sm font-medium">{t("Date applied")}</label>
                    <input type="date" value={form.date}
                      onChange={(e) => setForm((p) => ({ ...p, date: e.target.value }))}
                      className="w-full rounded-2xl border border-zinc-300 px-4 py-3 outline-none focus:border-zinc-900" required />
                  </div>
                  <div>
                    <label className="mb-2 block text-sm font-medium">{t("Method")} <span className="font-normal text-zinc-400">{t("(optional)")}</span></label>
                ...
                <div>
                  <label className="mb-2 block text-sm font-medium">{t("Spray / treatment")}</label>
                  <input type="text" value={form.product}
                    onChange={(e) => setForm((p) => ({ ...p, product: e.target.value }))}
                    className="w-full rounded-2xl border border-zinc-300 px-4 py-3 outline-none focus:border-zinc-900"
                    placeholder={t("Neem oil, garlic-chilli, soap spray…")} required />
                </div>
                <div>
                  <label className="mb-2 block text-sm font-medium">{t("Target pest")} <span className="font-normal text-zinc-400">{t("(optional)")}</span></label>
                  <input type="text" value={form.target_pest}
                    onChange={(e) => setForm((p) => ({ ...p, target_pest: e.target.value }))}
                    className="w-full rounded-2xl border border-zinc-300 px-4 py-3 outline-none focus:border-zinc-900"
                    placeholder={t("Aphids, cutworm…")} />
                </div>
                <div>
                  <label className="mb-2 block text-sm font-medium">{t("Quantity used")} <span className="font-normal text-zinc-400">{t("(optional)")}</span></label>
                  <input type="text" value={form.quantity}
                    onChange={(e) => setForm((p) => ({ ...p, quantity: e.target.value }))}
                    className="w-full rounded-2xl border border-zinc-300 px-4 py-3 outline-none focus:border-zinc-900"
                    placeholder={t("5 L, 2 knapsacks…")} />
                </div>
                <div>
                  <label className="mb-2 block text-sm font-medium">{t("Zones / Beds")} <span className="font-normal text-zinc-400">{t("(select multiple)")}</span></label>
                  <div className="space-y-2 rounded-2xl border border-zinc-300 p-3">
                    {zones.length === 0 ? (
                      <p className="text-sm text-zinc-400">{t("No zones available")}</p>
                    ) : (
                      zones.map((z) => (
                ...
                <div>
                  <label className="mb-2 block text-sm font-medium">
                    {t("Next time to spray")} <span className="font-normal text-zinc-400">{t("(optional)")}</span>
                  </label>
                  <input type="date" value={form.next_spray_date}
                    onChange={(e) => setForm((p) => ({ ...p, next_spray_date: e.target.value }))}
                    className="w-full rounded-2xl border border-zinc-300 px-4 py-3 outline-none focus:border-zinc-900" />
                  <p className="mt-1.5 text-xs text-zinc-400">
                    {t("Adds a goal on this date so you get reminded to spray again.")}
                  </p>
                </div>
                <div>
                  <label className="mb-2 block text-sm font-medium">{t("Notes")} <span className="font-normal text-zinc-400">{t("(optional)")}</span></label>
                  <textarea value={form.notes}
                    onChange={(e) => setForm((p) => ({ ...p, notes: e.target.value }))}
                    className="w-full rounded-2xl border border-zinc-300 px-4 py-3 outline-none focus:border-zinc-900"
                    rows={2} placeholder={t("Any extra details…")} />
                </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-100 text-left text-xs font-semibold uppercase tracking-[0.12em] text-zinc-500">
                  ...
                  <th className="px-5 py-4">{t("Spray / treatment")}</th>
                  <th className="px-5 py-4">{t("Target pest")}</th>
                  <th className="px-5 py-4">{t("Method")}</th>
                  <th className="px-5 py-4">{t("Quantity")}</th>
                  <th className="px-5 py-4">{t("Next spray")}</th>
                  <th className="px-5 py-4">{t("Zone / Bed")}</th>
                ...
                {entries.map((entry, i) => (
                  editingId === entry.id ? (
                    <tr key={entry.id} className="border-b border-zinc-100 bg-amber-50/40">
                      ...
                      <td className="px-3 py-2">
                        <div className="space-y-1 rounded border border-zinc-300 p-2">
                          {zones.length === 0 ? (
                            <p className="text-xs text-zinc-400">{t("No zones")}</p>
                          ) : (
                            zones.map((z) => (
                    ...
                    <tr key={entry.id} className={`border-b border-zinc-100 last:border-0 ${i % 2 === 0 ? "" : "bg-zinc-50/50"}`}>
                      ...
                      <td className="px-5 py-4 text-zinc-600">{entry.method ? t(entry.method) : "—"}</td>
