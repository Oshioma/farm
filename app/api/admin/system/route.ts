...
const ENV_SPECS: EnvSpec[] = [
  ...
  },
  {
    ...
    required: false,
    secret: true,
    purpose: "Sends password reset emails and the new-signup and join-request notifications.",
    impact: "Password resets fall back to Supabase's own email; signup and join-request emails are never sent.",
  ...
  },
  {
    ...
    required: false,
    secret: false,
    purpose: "Sender shown on emails the app sends, e.g. \"Shamba Online <hello@shamba.online>\". The domain must be verified in Resend.",
    impact: "Emails go out from onboarding@resend.dev, which only delivers to the Resend account owner.",
    validate: (v) => (v.includes("@") ? null : "Not an email address"),
  },
  {
    ...
    required: false,
    secret: false,
    purpose: "Public address of the site, used for the links inside emails, e.g. https://shamba.online.",
    impact: "Links are built from the address of the request that asked for them.",
    validate: (v) => (v.startsWith("https://") ? null : "Should start with https://"),
  },
  {
    key: "NOTIFY_EMAIL",
    group: "Email",
